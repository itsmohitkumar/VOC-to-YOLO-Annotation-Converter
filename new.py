...
# Removed image_generator node from flow
...
# End regeneration after platform-specific agent
...
def enhance_prompt(prompt: str, state: State) -> str:
    attempt = state.get("regen_attempt_number", 1)
    current_post_id = state.get("current_post_id", "")
    lines = [f"--- REGENERATION ATTEMPT {attempt} FOR POST {current_post_id} (MUST BE DISTINCTLY DIFFERENT) ---"]
    strength = state.get("feedback_strength", "medium")
    if strength == "strong":
...
def create_social_media_post(state: State, platform: str) -> Dict[str, Any]:
    ...
    response = {
        post_key: post,
        "messages": [
            f"Enhanced {platform} post for day {current_day} (attempt {regen_attempt}) created with temp {temperature:.2f} using AWS Bedrock"
        ],
...
async def submit_feedback(
    ...
    try:
        ...
        # Deduplicate feedback items by post_id (case-insensitive)
        deduped_feedbacks = []
        ...
        # Load stored plan and feedback tracking from DB
        ...
        # Load feedback tracking from DB or initialize
        feedback_dict = {}
        ...
        # Normalize regeneration attempt keys
        ...
        # Load plan state dict or fallback
        state_dict = {}
        ...
        # Helper functions for feedback strength, temperature, seed, similarity
        ...
        for fb in deduped_feedbacks:
            ...
            logger.info(f"Processing feedback for post_id: {post_id}")

            # Parse post_id components
            ...
            # Check regeneration attempt limits
            current_regen_attempts = feedback_dict.get("current_regen_attempts", {})
            ...
            # Record feedback and increment attempt count
            feedback_dict.setdefault("feedback_history", []).append({
                "post_id": post_id,
                "timestamp": datetime.utcnow().isoformat(),
                "feedback_text": fb_text,
            ...
            logger.info(f"Constructing regeneration state for {post_id}")
            ...
            # Merge states
            state_input = {}
            state_input.update(state_dict)
            state_input.update(feedback_dict)
            state_input.update(base_regeneration_state)
            ...
            for retry in range(max_retry_attempts):
                try:
                    ...
                    logger.info(f"Invoking system agents for {post_id} attempt {retry + 1}")
                    ...
                    # Attempt 1: extract from campaign_plan structure
                    ...
                    # Attempt 2: fallback direct extraction
            ...
            # Merge regenerated content into full plan
            if isinstance(new_day_node, dict):
                ...
                # Merge fallback keys
                fallback_keys = ("task", "content", "caption", "headline", "cta", "hook", "angle", "hashtags", "title", "body", "notes")
                for k in fallback_keys:
                    if k in final_state_dict:
                        merged_node[k] = final_state_dict[k]

            # Clean image-related fields
            for img_key in ["image_path_s3", "image_paths", "image_urls", "images", "image_prompt"]:
            ...
            # Store variant snapshot for similarity checks
            pv_map = feedback_dict.setdefault("previous_variants", {})
            pv_list = pv_map.get(pid_norm, [])
            variant_snapshot = {k: merged_node.get(k) for k in ("content", "caption", "headline", "cta", "hook", "angle", "hashtags", "title", "body", "notes") if k in merged_node}
            ...
            details[post_id] = {
                ...
                "feedback_strength": feedback_strength,
                "temperature_used": temperature,
                "new_content_preview": merged_node.get("content", "")[:100] + "..."
            }
        ...
        # Update feedback tracking in DB
        ...
        # Update database with feedback response and approved plan versions
        try:
