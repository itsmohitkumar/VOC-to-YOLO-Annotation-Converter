...
# Initialize S3 client using environment variables (global)
...
def extract_content_and_images(content: str) -> tuple:
    ...
    # Patterns to identify image descriptions (fixed regex typo)
    image_patterns = [
        r'\*\*Image:\*\*[^\n]*',
        r'\*\*Visuals:\*\*[^\n]*',
        r'\(Image:[^)]*\)',
...
def upload_excel_to_s3(excel_content: bytes, s3_key: str, bucket: str) -> str:
    ...
    """
    try:
        # Use global s3_client (removed redundant init)
...
def generate_plan_overview_excel(campaign_plan: Dict[str, Any], writer) -> None:
    ...
    # Create DataFrame and save to Excel with "Campaign Plan" sheet name (only if data exists)
    if overview_
        df_overview = pd.DataFrame(overview_data)
        df_overview.to_excel(writer, sheet_name='Campaign Plan', index=False)
    else:
        # Add a dummy sheet to avoid IndexError if no data
        pd.DataFrame([{'Note': 'No data available'}]).to_excel(writer, sheet_name='Campaign Plan', index=False)

def generate_content_excel(campaign_plan: Dict[str, Any], writer, plan_ Dict[str, Any] = None) -> None:  # Added plan_data param
    has_data = False
    for platform, weeks in campaign_plan.items():
        content_data = []
        for week_key, days in weeks.items():
    ...
    # If no data at all, add a dummy sheet to avoid errors
    if not has_
        pd.DataFrame([{'Note': 'No content data available'}]).to_excel(writer, sheet_name='Content', index=False)
